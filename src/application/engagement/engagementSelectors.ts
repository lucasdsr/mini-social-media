...
export const selectPostsWithEngagement = createSelector(
  ...
  (posts, commentsByPost) => calculateEngagementScores(posts, commentsByPost)
)
...
export const selectEngagementStats = createSelector(
  ...
  posts => {
    ...
    const levelCounts = posts.reduce(
      (acc, post) => {
        if ('engagementScore' in post) {
          const level = (post as PostWithEngagementScore).engagementScore.level
